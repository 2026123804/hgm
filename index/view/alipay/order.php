                ...
                <div class="col s12 m12 l12">
                  ...
                  <div class="input-field col s6" style="font-weight:normal;">
                    ...
                    <label>选择通道来查看<?php if ($_SESSION['ALIPAY']['ORDER']['WHERE'] == ''){?>(<a href="#" onclick="alipay();">开始查询</a>)<?php }else{?>(<a href="<?php echo url::s('index/alipay/automaticOrder',"sorting=alipay&locking=closed");?>">取消锁定</a>)<?php }?></label>
                        ...
                        <th>支付信息 <?php if ($sorting['code'] != 0 && $sorting['name'] == 'status'){?>(<?php if ($sorting['code'] == 1) echo '获取订单中';if ($sorting['code'] == 2) echo '未支付';if ($sorting['code'] == 3) echo '订单超时';if ($sorting['code'] == 4) echo '已支付';?>)<?php }?><a href='<?php echo url::s('index/alipay/automaticOrder',"sorting=status&code=".($sorting['code']+1));?>'><i class="mdi-image-healing"></i></a></th>
                    ...
                    <?php foreach ($result['result'] as $ru){?>
                      <tr>
                        <td>支付宝ID：<a href='<?php echo url::s("index/alipay/automatic");?>'><?php echo $ru['alipay_id'];?></a> / 订单ID：<?php echo $ru['id'];?> ( <a target="_blank" href="<?php echo url::s('gateway/pay/automaticAlipay',"id={$ru['id']}");?>">支付链接</a> )
